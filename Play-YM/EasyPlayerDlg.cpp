...
BOOL CLivePlayerDlg::OnInitDialog()
{
	...
	{
		pVideoWindow->channels = proConfig.splitWindow;
		pVideoWindow->layout3x10 = (proConfig.splitWindow == 30);  // 添加3x10布局判断
	}
	if (NULL != pComboxSplitScreen)
	{
		...
		else if (24 == proConfig.splitWindow)     pComboxSplitScreen->SetCurSel(4);
		else if (30 == proConfig.splitWindow)     pComboxSplitScreen->SetCurSel(5);
		else if (36 == proConfig.splitWindow)     pComboxSplitScreen->SetCurSel(6);
		else if (64 == proConfig.splitWindow)     pComboxSplitScreen->SetCurSel(7);
	}
...
void	CLivePlayerDlg::CreateComponents()
{
	...
	if (NULL == pVideoWindow)
	{
		...
		pVideoWindow->layout3x10    = false;
		pVideoWindow->layout8x3     = false; // 初始化8x3布局标志
	...
	if (NULL != pComboxSplitScreen)
	{
		pComboxSplitScreen->AddString(TEXT("4 布局"));
		pComboxSplitScreen->AddString(TEXT("8 布局"));
		pComboxSplitScreen->AddString(TEXT("9 布局"));
		pComboxSplitScreen->AddString(TEXT("16布局"));
		pComboxSplitScreen->AddString(TEXT("24布局")); // 添加新的24格布局选项
		pComboxSplitScreen->AddString(TEXT("30布局"));
		pComboxSplitScreen->AddString(TEXT("36布局"));
		pComboxSplitScreen->AddString(TEXT("64布局"));
...
void	CLivePlayerDlg::UpdateVideoPosition(LPRECT lpRect)
{
	...
	if (pVideoWindow->maximizedId==-1)
	{
		...
		// 添加8x3布局处理
		if (pVideoWindow->layout8x3)
		{
			RECT rcTmp;
			SetRectEmpty(&rcTmp);

			// 计算8x3布局的窗口尺寸
			int cellWidth = rcClient.Width() / 8;
			int cellHeight = rcClient.Height() / 3;
			...
			for (int row = 0; row < 3; row++)
			{
				...
				{
					int left = rcClient.left + (col * cellWidth);
					int top = rcClient.top + (row * cellHeight);
					
					SetRect(&rcTmp, left, top, left + cellWidth, top + cellHeight);
					
					// 处理最右列和最底行的边缘对齐
					if (col == 7 && rcTmp.right < rcClient.right)
						rcTmp.right = rcClient.right;
					if (row == 2 && rcTmp.bottom < rcClient.bottom)
					...
					n++;
					if (n >= 24) break; // 最多显示24个窗口
				}
				if (n >= 24) break;
			}
		}
		else if (pVideoWindow->layout3x10)
		{
			RECT rcTmp;
			SetRectEmpty(&rcTmp);

			// 计算3x10网格的窗口尺寸
			int cellWidth = rcClient.Width() / 10;
			int cellHeight = rcClient.Height() / 3;
			...
			for (int row = 0; row < 3; row++)
			{
				for (int col = 0; col < 10; col++)
				{
					...
					// 处理最后一列和最后一行的边缘对齐
					if (col == 9 && rcTmp.right < rcClient.right)
						rcTmp.right = rcClient.right;
					if (row == 2 && rcTmp.bottom < rcClient.bottom)
						rcTmp.bottom = rcClient.bottom;
...
void CLivePlayerDlg::OnCbnSelchangeComboSplitScreen()
{
    ...
    else if (nIdx == 4) nSplitWindow = 24; // 24格布局
    else if (nIdx == 5) nSplitWindow = 30;
    else if (nIdx == 6) nSplitWindow = 36;
    else if (nIdx == 7) nSplitWindow = 64;

    pVideoWindow->channels = nSplitWindow;
    pVideoWindow->layout3x10 = (nSplitWindow == 30);
    pVideoWindow->layout8x3 = (nSplitWindow == 24); // 设置8x3布局标志

    // 保存当前配置到配置文件
    XMLConfig xmlConfig;
    PRO_CONFIG_T config;
    memset(&config, 0x00, sizeof(PRO_CONFIG_T));
    xmlConfig.LoadConfig(XML_CONFIG_FILENAME, &config);
    config.splitWindow = nSplitWindow;
    xmlConfig.SaveConfig(XML_CONFIG_FILENAME, &config);
