...
int		XMLConfig::LoadConfig(char *filename, PRO_CONFIG_T *pConfig)
{
	...
	if (! m_DocR.LoadFile(filename))
	{
		...
		proConfig.splitWindow = 16;  // 设置默认值为16分屏
		proConfig.scale	=	0;
	...
	if (NULL != pConfigXML)
	{
		TiXmlElement *pE = pConfigXML->FirstChildElement("SplitWindow");
		if (pE && pE->GetText())
		{
			int splitWindow = atoi(pE->GetText());
			// 验证分屏值的有效性
			if (splitWindow == 4 || splitWindow == 8 || splitWindow == 9 || 
				splitWindow == 16 || splitWindow == 30 || splitWindow == 36 || 
				splitWindow == 64)
			{
				proConfig.splitWindow = splitWindow;
			}
			else
			{
				proConfig.splitWindow = 16; // 无效值使用默认值
			}
		}
